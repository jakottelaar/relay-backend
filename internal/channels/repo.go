...
func (r *channelsRepo) SaveChannel(ctx context.Context, channel *Channel) (*Channel, error) {
	query := `
        INSERT INTO channels (name, owner_id, channel_type)
        VALUES ($1, $2, $3)
	...
	row := r.db.QueryRowContext(ctx, query, channel.ID, channel.Name, channel.OwnerID, channel.ChannelType, channel.CreatedAt, channel.UpdatedAt)
